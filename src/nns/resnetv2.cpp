...
static constexpr int seg_lens_v2_50[] = {3,4,6,3};
static constexpr int seg_lens_v2_101[] = {3,4,23,3};
static constexpr int seg_lens_v2_152[] = {3,8,36,3};

// Pre-activation Bottleneck block for ResNetV2.
static void btn_blk_v2(
	...
	lid_t layer_a = n.add(NLAYER(name+"a", Conv, C=in_C, K=mid_C, H=H_len, sH=strd), {prev});
	lid_t layer_b = n.add(NLAYER(name+"b", Conv, C=mid_C, K=mid_C, H=H_len, R=3), {layer_a});
	lid_t layer_c = n.add(NLAYER(name+"c", Conv, C=mid_C, K=out_C, H=H_len), {layer_b});
	if(has_br){
...
static Network gen_resnetv2(const int* seg_lens){
	...
	for(int seg=2; seg<6; ++seg){
		...
		for(int blk=0; blk<seg_lens[seg-2]; ++blk){
			btn_blk_v2(n, prev, seg, blk, blk==0,
...
const Network resnetv2_50 = gen_resnetv2(seg_lens_v2_50);
const Network resnetv2_101 = gen_resnetv2(seg_lens_v2_101);
const Network resnetv2_152 = gen_resnetv2(seg_lens_v2_152);